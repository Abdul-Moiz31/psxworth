...
function Slot<T extends HTMLElement = HTMLElement>({ children, ref, ...props }: SlotProps<T>) {
  ...
  const element = children as React.ReactElement<AnyProps>;
  const mergedRef = mergeRefs(childRef as React.Ref<T>, ref);

  if (typeof element.type === "string") {
    const MotionTag = (motion as unknown as Record<string, React.ElementType>)[element.type] ?? motion.div;
    return <MotionTag {...mergedProps} ref={mergedRef} />;
  }

  return React.cloneElement(element, mergedProps as AnyProps);
}
